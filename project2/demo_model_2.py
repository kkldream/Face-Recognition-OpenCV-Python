...
def main():
    ...
    while True:
        ...
        for face_result in face_results:
            ...
            Test = np.array([
                norm_max_min(mar, 0, 1),
                norm_max_min(ear[0], 0, 1),
                norm_max_min(ear[1], 0, 1),
                norm_max_min(rotation[1], -50, 50),
                norm_max_min(H_f, 0, 30)
            ])
            TestList[:-1] = TestList[1:]
            TestList[-1] = Test
            predicted = my_model.predict([
                np.array([TestList[:, 0]]),
                np.array([TestList[:, 1:4]]),
                np.array([np.average(TestList[:, 4])])
            ])
